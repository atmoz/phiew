...
class Phiew_View_Helper_InsertDateSelect
{
	...
	public function insertDateSelect($name = null, $value = array(),
		...
		if ($checkbox)
		{
			...
			$toggleActiveHtml = '<input type="checkbox" name="'.$name.'[active]" value="1" '.$checked.' />';
		}
		else
		{
			$active = (int)(bool) (isset($value['active'])) ? $value['active'] : null;
			$toggleActiveHtml = '<input type="hidden" name="'.$name.'[active]" value="'.$active.'" />';
		}

		$html = '<span class="ViewHelper_DateTime"><input type="hidden" name="'.$name.'[dummy]" value="1" />' 
			  . sprintf($format, $toggleActiveHtml, $dayHtml, $monthHtml, $yearHtml) . '</span>';
